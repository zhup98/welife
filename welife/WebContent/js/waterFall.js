...
function waterFlow(parent, chirld) {
	...
	var wchirldWidth = wparent.getElementsByClassName("pin");
	// 这是一个Math算法,目的是将小数转变为整数,从而可以知道每行最多容纳几张图片
	var num = Math.floor(wscreenWidth / wchirldWidth[0].offsetWidth);
	// 固定每行摆放个数 和上下左右边距
	wparent.style.cssText = 'width:' + wchirldWidth[0].offsetWidth * num
			+ 'px;margin:0 auto';
	// 获得每行的最小高度
...
function getMinHeightOfCols(chirdArr, num) {
	...
	for (var i = 0; i < chirdArr.length; i++) {
		...
		if (i < num) {
			...
			// 得到下一行图片应放于那个位置
			chirdArr[i].style.left = chirdArr[minHeightOfindex].offsetLeft
					+ "px";
